...
class FMAgent():
    ...
    def policy(self, s):
        if np.random.random() < self.epsilon or not self.initialized:
            ...
            if self.estimate_prob:
                action = np.random.choices(self.actions, 
                                           size=1, 
                                           p=estimates)[0]
                return action
            else:
...
def train_loop(self, env, agent, episode=200,
               initial_count=-1, render=False, obeserve_interval=0):
    ...
    for i in range(episode):
        ...
        while not done:
            if render:
                env.render()
            if self.training and obeserve_interval > 0 and \
                (self.training_count == 1 or 
                 self.training_count % obeserve_interval == 0):
                frames.append(s)
                a = agent.policy(s)                
                n_state, reward, done, info = env.step(a)
...
def get_recent(self, count):
    ...
    [self.experiences[i] for i in recent]
...
class Logger():
    ...
    def plot(self, name, values, interval=-10):
        ...
        plt.grid()
        plt.fill_between(indices, means - stds,
